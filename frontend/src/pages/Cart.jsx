      ...
      <div>
        {
          cartData.map((item, index) => {
            ...
            return (
              <div key={index} className='py-4 border-t border-b text-gray-700 dark:text-gray-100 border-gray-300 dark:border-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
                
                {/* Product Image & Info */}
                <div className='flex items-start gap-6'>
                  <img className='w-16 sm:w-20 rounded-md dark:border dark:border-gray-600' src={productData.image[0]} alt="" />
                  <div>
                    <p className='text-xs sm:text-lg font-medium dark:text-gray-100'>{productData.name}</p>
                    <div className='flex items-center gap-5 mt-2'>
                      <p>{currency}{productData.price}</p>
                      <p className='px-2 sm:px-3 sm:py-1 border bg-slate-50 dark:bg-gray-800 dark:text-white dark:border-gray-600'>{item.size}</p>
                    </div>
                  </div>
                </div>
